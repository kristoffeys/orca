...
describe('Store', () => {
  ...
  it('normalizes malformed visible task providers on load', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().visibleTaskProviders).toEqual(['gitlab', 'jira', 'productive'])
  })
  ...
  it('preserves a deliberate Jira provider opt-out after migration', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().visibleTaskProviders).toEqual(['gitlab', 'productive'])
  })

  it('preserves a deliberate Productive provider opt-out after migration', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
