...
export type ProductiveTaskProviderIdentity = {
  provider: 'productive'
...
export type TaskProviderIdentity =
  ...
  | ProductiveTaskProviderIdentity
...
export function isStoredTaskProviderIdentity(provider: TaskProvider, identity: unknown): boolean {
  ...
  switch (provider) {
    ...
    case 'productive':
      return ['organizationId', 'projectId'].every((key) => isNullableOptionalString(raw[key]))
  }
}
...
const TASK_PROVIDER_IDENTITY_FIELDS: Record<TaskProvider, readonly string[]> = {
  ...
  productive: ['organizationId', 'projectId']
}
...
export function taskProviderIdentityCachePart(
  ...
  switch (identity.provider) {
    ...
    case 'productive':
      return [identity.organizationId, identity.projectId].filter(Boolean).join('/')
  }
}
