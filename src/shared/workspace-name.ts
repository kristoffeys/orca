...
function compactWorkItemTitle(title: string, item: WorkspaceIntentWorkItem): string {
  const identifier = item.linearIdentifier ?? item.jiraIdentifier ?? item.productiveIdentifier
...
function workItemIdentity(item: WorkspaceIntentWorkItem): string {
  ...
  if (item.productiveIdentifier) {
    return item.productiveIdentifier.toUpperCase()
  }
  if (item.type === 'pr') {
