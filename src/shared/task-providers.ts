export type TaskProvider = 'github' | 'gitlab' | 'linear' | 'jira' | 'productive'
...
function isTaskProviderAvailable(
  ...
  // Why: Productive (like Jira) is connected from the Tasks surface itself, so it
  // must stay visible while disconnected to keep the first-time setup entry point.
  if (provider === 'productive') {
    return true
  }
